...
class Processor:
    ...
    def process(self, json_string: str):
        ...
        logger.debug(f"pr_number {pr_number}: selection_strategy chosen: {selection_strategy}")
        ...
        match selection_strategy:
            case SelectionStrategy.OPENAI:
                ...
                logger.debug(f"pr_number {pr_number}: published request to OPENAI reviewer recommendation request topic")
            case SelectionStrategy.HEURISTIC:
                ...
                logger.debug(f"pr_number {pr_number}: published request to HEURISTIC reviewer recommendation request topic")
            case _:

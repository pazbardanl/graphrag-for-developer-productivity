...
def _get_kafka_config():
    ...
    reviewer_recommendation_requests_heuristic = os.environ["KAFKA_REVIEWER_RECOMMENDATION_REQUESTS_HEURISTIC_TOPIC"]
    group_id = os.environ["KAFKA_REVIEWER_RECOMMENDATION_REQUEST_DISPATCHING_SERVICE_GROUP_ID"]
    return kafka_bootstrap_servers, new_pr_events_topic, reviewer_recommendation_requests_openai, reviewer_recommendation_requests_heuristic, group_id
...
def main():
    logger.info("started")
    kafka_bootstrap_servers, new_pr_events_topic, reviewer_recommendation_requests_openai, reviewer_recommendation_requests_heuristic, group_id = _get_kafka_config()
    user_data_provider = UserDataProvider("http://user-data-service:8080")
    ...
    reviewer_recommendation_request_heuristic_publisher = Publisher(kafka_bootstrap_servers, reviewer_recommendation_requests_heuristic)
    processor = Processor(user_data_provider, reviewer_recommendation_request_openai_publisher, reviewer_recommendation_request_heuristic_publisher)
